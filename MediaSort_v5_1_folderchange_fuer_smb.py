...
changelog _fuer_smb 2021/08/11:
    HIer musste ein SUDO dals Superuser rein, sonst gibt es kein Schrreibrecht. Dadurch gint es das umbenenn und 
    Folder Create nur auf Systemebene und dasfür muss wiederum beim Erstellen des Ordner der "/" vorn wen (deshalab pfad2)
    Problem wenn der hauptpfad "jpeg" nicht vorhanden ist, muss der auch erstellt werden, dafür müsste der "/" wieder davor
    Das gilt es noh zu lösen
        ...
        for i in filter(os.path.isfile, os.listdir(direc)):  # Searches for media files in folder
            try:
                ...
                if endung.lower() in endungen_exif:  # The if in part is looking for correspodent file extensions in
                    ...
                    print "shtime ", shtime
                    pfad = os.path.join("/", endungen_exif[endung.lower()], shtime) #für sudo auskommentioert, da der vordere Slash nicht geht,
                    pfad2 = os.path.join(endungen_exif[endung.lower()], shtime)
                    print "pfad ", pfad
                    if not os.path.exists(pfad):
                        # os.makedirs(pfad)
                        print "hier dann wird pfad2 " +pfad2
                        os.system("sudo -S mkdir -p {}".format(pfad2))
                        print "nach mkdir"
                    shutil.move(imagepath, pfad2)
